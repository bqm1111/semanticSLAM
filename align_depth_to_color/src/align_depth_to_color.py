...
class AlignDepth:
    def __init__(self):
        rospy.init_node("align_depth_to_color", anonymous=False)
        ...
        self.tf_color_frame = rospy.get_param("d455_color_optical_frame")
        self.tf_depth_frame = rospy.get_param("d455_depth_optical_frame")
        ...
        self.aligned_depth_pub = rospy.Publisher(
            "/aligned_depth_to_color", Image, queue_size=10
        )

        self.bridge = CvBridge()
        self.depth_to_color_transform = get_transform(
            target_frame=self.tf_color_frame, source_frame=self.tf_depth_frame
        )
    ...
    def image_callback(
        ...
        start = time.time()
        aligned_depth = depth_to_color_frame(
            depth_img, 0.001, K_depth, K_color, self.depth_to_color_transform
        )
        print("=============> Time elapsed = ", time.time() - start)
        try:
            aligned_depth_msg = self.bridge.cv2_to_imgmsg(
                aligned_depth, encoding="32FC1"
            )
...
if __name__ == "__main__":
    rospy.loginfo("Starting depth alignment node")
    node = AlignDepth()
    rospy.spin()
