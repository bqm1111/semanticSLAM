...
namespace octomap
{
	...
	bool SemanticsOcTree<SEMANTICS>::isNodeCollapsible(const SemanticsOcTreeNode<SEMANTICS> *node) const
	{
		...
		for (unsigned int i = 1; i < 8; i++)
		{
			...
			if (!this->nodeChildExists(node, i) || this->nodeHasChildren(this->getNodeChild(node, i)) || !(this->getNodeChild(node, i)->getValue() == firstChild->getValue()) || !(this->getNodeChild(node, i)->getSemantics() == firstChild->getSemantics()))
				return false;
		}
		return true;
	}
